...
{
	// On laisse la fonction originale créer le périphérique
	...
	if (SUCCEEDED(hres))
	{
		// On charge notre shader de végétation
		LoadVegetationShader(*ppReturnedDeviceInterface);

		// On crée notre proxy pour envelopper le périphérique original
		IDirect3DDevice9Proxy* tmp = new IDirect3DDevice9Proxy(*ppReturnedDeviceInterface);
		*ppReturnedDeviceInterface = tmp;

		// --- SOLUTION : On force un Reset immédiat ---
		// Cela force le périphérique à se resynchroniser avec la fenêtre et corrige souvent les bugs d'affichage au démarrage.
